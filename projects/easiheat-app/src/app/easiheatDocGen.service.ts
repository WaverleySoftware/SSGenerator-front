...
@Injectable()
export class easiHeatDocGenService {

  constructor(private http: HttpClient) {

  }

   getEasiHeatPdf(docGen: DocGen) {
    let options = {
      headers: {
     ...
     this.http.post<any>('./Api/reports/easiHeat/DocGen/CacheModel', docGen).subscribe(response => {     

      var sessionId = response.sessionId;

       window.open('./Api/reports/easiHeat/DocGen/ReportViewer?id=' + sessionId);

...
export class SetUnitsDirective implements OnInit {
  ...
  get preferences(): Preference[] {
    ...
    for (const preferenceName of this.preferenceNames) {
      if (preferenceName) {
        preferences.push(this.getSizingPreference(preferenceName));
      }
    }
  ...
  private getSizingPreference(name?: string): Preference | null {
    ...
    const preference = this.preferenceService.sizingUnitPreferences.find((p) => p && p.preference && p.preference.name === name);

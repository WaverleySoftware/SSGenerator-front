...
import { SgaHttpValidationResponseInterface, SteamGeneratorInputsInterface } from '../interfaces/steam-generation-form.interface';
...
export class SgaValidator {
  ...
  private static _parseErrors(response: SgaHttpValidationResponseInterface): ValidationErrors {
    ...
    return {
      error,
      message: errors[0] && (errors[0].customState || errors[0].customState === 0) && `(${Math.round(errors[0].customState)})`
    };
  }

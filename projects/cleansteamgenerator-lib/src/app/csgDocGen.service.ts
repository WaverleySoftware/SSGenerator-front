  ...
  constructor(private http: HttpClient) {
   ...
   getCSGPdf(docGen: DocGen) {
    let options = {
     ...
     this.http.post<any>('./Api/reports/Csg/DocGen/CacheModel', docGen).subscribe(response => {     

      var sessionId = response.sessionId;

       window.open('./Api/reports/Csg/DocGen/ReportViewer?id=' + sessionId);
